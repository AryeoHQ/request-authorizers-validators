...
A custom PHPStan rules is available to add to your projects to ensure authorizers and validators follow the implementation standards.

The rule can be added to your projects `phpstan.neon` to the `rules` key.

```yml
rules:
    - Tooling\PHPStan\Rules\AuthorizerValidatorRule
```
...
return RectorConfig::configure()
    ->withRules([
        FixAuthorizorsAndValidators::class
        //..
    ])
    //..
```